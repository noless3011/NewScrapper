			...
			for (WebElement element : elements) {
				try {
					...
					if (dauthors.size() > 0) {
						...
						dauthor = element
								.findElement(By.cssSelector("div.typography__StyledTypography-sc-owin6q-0.gzrhkO"));
					}
					String author = dauthor.getText();

					WebElement ddate = element
							.findElement(By.cssSelector("h6.typography__StyledTypography-sc-owin6q-0.jjSgIS"));
					...
					if (!uniquearticles.contains(url) && content != null) {
...
//    					callback.updateProgress(index);
			...
			i++;
			List<WebElement> page = mainDriver.findElements(By.xpath(
					"//button[@class='Button__ButtonBase-sc-1sh00b8-0 Button__TextButton-sc-1sh00b8-1 gjtbfz ccioqw searchstyles__PageButton-sc-ci5zlg-17 dZKZem']"));
	...
	private Content extractContentFromUrl(String url) {
		...
		try {
			String imgUrl = articleDriver
					.findElement(By.xpath(
							"//*[@id='article-header']/div/header/div[2]/div/div/div[1]/div[6]/div/figure/picture/img"))
					.getAttribute("src");
		...
		try {
			WebElement articleBody = articleDriver
					.findElement(By.xpath("//div[@data-submodule-name='composer-content']"));
			...
			for (WebElement element : allchildElement) {
				...
				if (checkElementClass(element, "imagestyles__StyledWrapper-sc-18f23w7-0 kRaHHW")) {
					String imageUrl = element.findElement(By.xpath(".//div[@class='at-img']/figure/picture/img"))
							.getAttribute("src");
