...
public class TwitterCrawler implements ICrawler<Tweet> {
	...
	public void loginTwitter() throws InterruptedException {
		...
		boolean phoneNumberRequired = isElementPresent(driver,
...
//		while(!isElementPresent(driver, By.xpath("//span[contains(text(), 'Something went wrong. Try reloading.')]"))) {
			...
			try {
				...
				try {
					...
					for (WebElement tweet : tweets) {
						...
						String author = tweet
								.findElement(
										By.xpath(".//span[@class='css-1jxf684 r-bcqeeo r-1ttztb7 r-qvutc0 r-poiln3']"))
								.getText();
						...
						String number_of_comment = tweet.findElement(By.xpath(".//button[@data-testid='reply']"))
								.getText();
						if (number_of_comment.equals(""))
							number_of_comment = "0";

						// Lấy số lượt thích
						String number_of_liked = tweet.findElement(By.xpath(".//button[@data-testid='like']"))
								.getText();
						if (number_of_liked.equals(""))
							number_of_liked = "0";

						// Lấy số lượt xem
						String number_of_view = tweet.findElement(By.xpath(
								".//a[@class='css-175oi2r r-1777fci r-bt1l66 r-bztko3 r-lrvibr r-1ny4l3l r-1loqt21']"))
								.getText();
						if (number_of_view.equals(""))
							number_of_view = "0";
						...
						String timeStringFromTwitter = tweet
								.findElement(By.xpath(".//div[@class='css-175oi2r r-18u37iz r-1q142lx']//a/time"))
	...
	public Content crawlTweetContent(WebElement tweet) {
		try {
			...
			// Lấy url ảnh
			List<WebElement> imgElements = tweet
					.findElements(By.xpath(".//img[@alt='Image' and not(contains(@src,'profile_images'))]"));
